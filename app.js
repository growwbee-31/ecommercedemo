...
async function renderAccount() {
  ...
  const ordersHtml = state.userOrders.length === 0
    ? '<p class="notice">You have not placed any orders yet.</p>'
...
async function renderOrders() {
  ...
  await loadUserOrders();
  const ordersHtml = state.userOrders.length === 0
    ? '<p class="notice">You have no orders yet.</p>'
    : state.userOrders.map(order => `
      <div class="panel">
        <h3>Order #${order.id}</h3>
        <p>Total: ₹${Number(order.total).toFixed(2)}</p>
  ...
  app.innerHTML = `
    <section class="section-title">
      <div>
        <h1>My Orders</h1>
        <p class="notice">View the status of your orders and see your latest purchases.</p>
      </div>
    </section>
    <div>${ordersHtml}</div>
  `;
...
async function renderAdmin() {
  ...
  const ordersHtml = state.orders.length === 0
    ...
    : state.orders.map(order => `
      <div class="panel">
        <h3>Order #${order.id}</h3>
        <p>User: ${escapeHtml(order.user_id || 'unknown')}</p>
        <p>Total: ₹${Number(order.total).toFixed(2)}</p>
        <p>Status: ${escapeHtml(order.status || 'pending')}</p>
        <p>Items: ${escapeHtml(order.items.map(item => `${item.quantity}× ${item.name}`).join(', '))}</p>
        <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:12px;">
          ${order.status !== 'confirmed' && order.status !== 'completed' ? `<button class="button" onclick="changeOrderStatus(${order.id}, 'confirmed')">Confirm</button>` : ''}
          ${order.status !== 'completed' ? `<button class="button" onclick="changeOrderStatus(${order.id}, 'completed')">Complete</button>` : ''}
          ${order.status !== 'cancelled' ? `<button class="button" style="background:#ef4444;color:#fff;" onclick="changeOrderStatus(${order.id}, 'cancelled')">Cancel</button>` : ''}
        </div>
      </div>
    `).join('');
...
async function loadUserOrders() {
  ...
  const { data, error } = await supabase.from('orders').select('*').eq('user_id', state.user.id).order('created_at', { ascending: false });
  if (error) {
    console.warn('Could not load user orders', error.message);
    state.userOrders = [];
    return;
  }
